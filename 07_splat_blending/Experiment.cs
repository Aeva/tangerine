...
public class Experiment : Game
{
    ...
    private VertexBuffer ScreenBuffer;
    private RenderTarget2D AccRenderTarget;
    ...
    protected override void LoadContent()
    {
        ...
        {
            byte[] ShaderBytes = File.ReadAllBytes("Shaders/AlphaDivide.ogl.mgfxo");
            AlphaDivideEffect = new Effect(GraphicsDevice, ShaderBytes);
            AlphaDivideEffect.CurrentTechnique = AlphaDivideEffect.Techniques[0];
        }
        ...
        {
            var ScreenPosition = new VertexDeclaration(
                new VertexElement(0, VertexElementFormat.Vector4, VertexElementUsage.Position, 0));

            var ScreenVertices = new Vector4[3];
            for (int VertexID = 0; VertexID < ScreenVertices.Length; ++VertexID)
            {
                float X = -1.0f + (float)((VertexID & 1) << 2);
                float Y = -1.0f + (float)((VertexID & 2) << 1);
                ScreenVertices[VertexID] = new Vector4(X, Y, 0.0f, 1.0f);
            }

            ScreenBuffer = new VertexBuffer(
                GraphicsDevice,
                ScreenPosition,
